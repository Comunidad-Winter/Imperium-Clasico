...
Public Function Char_Techo() As Boolean
    ...
    With charlist(UserCharIndex)
        ...
        If (Map_InBounds(.Pos.X, .Pos.Y)) Then '// Posicion valida
            ...
            'If (MapData(.Pos.X, .Pos.Y).Trigger = eTrigger.BAJOTECHO Or MapData(.Pos.X, .Pos.Y).Trigger = eTrigger.CASA Or MapData(.Pos.X, .Pos.Y).Trigger = eTrigger.ZONASEGURA) Then
...
Sub Char_MovebyHead(ByVal CharIndex As Integer, ByVal nHeading As E_Heading)
    ...
    With charlist(CharIndex)
        ...
        If CharIndex = UserCharIndex Then
            ...
            If frmMain.UltPos = 0 Then
                If ClientSetup.VerLugar = 1 Then _
                    frmMain.MapExp(0).Caption = "Posición: " & UserMap & ", " & .Pos.X & "  " & .Pos.Y
                    
            ElseIf ClientSetup.VerLugar = 0 Then
                frmMain.MapExp(0).Caption = "Posición: " & UserMap & ", " & .Pos.X & "  " & .Pos.Y
