...
Public Sub Map_CreateObject(ByVal X As Byte, ByVal Y As Byte, ByVal GrhIndex As Long, ByVal ParticulaIndex As Integer, ByVal Rango As Byte, ByVal Color As Long)
    ...
    '¿El objeto no tiene un Grh valido ni particula?
      If Not GrhCheck(GrhIndex) Then
            GrhIndex = 0 'Lo reemplazamos por un grafico invisible
            'Exit Sub
      End If
      ...
      If (Map_InBounds(X, Y)) Then
            ...
            With MapData(X, Y)
                ...
                If Rango > 0 Then
                    Call Long_To_RGB(tR, tG, tB, Color)
                    Call Create_Light_To_Map(X, Y, Rango, tR, tG, tB)
                End If
